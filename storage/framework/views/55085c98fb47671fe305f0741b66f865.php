...
<style>
    ...
    @media(max-width:1024px) { .product-layout { grid-template-columns: 1fr; gap: 2rem; } .product-img-wrap { position: static; } .related-grid { grid-template-columns: repeat(2,1fr); } }
</style>
<?php $__env->stopPush(); ?>
    ...
    <div class="product-detail">
        <div class="product-layout">
            ...
            <div class="product-img-wrap">
                <div class="product-main-img">
                    <?php if($product->product_image): ?>
                        <img src="<?php echo e(asset('storage/'.$product->product_image)); ?>" alt="<?php echo e($product->product_name); ?>" id="main-img">
                    <?php else: ?>
            ...
            <div class="product-info">
                <span class="product-cat-badge"><?php echo e($product->category->category_name ?? ''); ?></span>
                <?php if($product->is_featured): ?><span class="badge badge-gold" style="margin-left:.4rem;">Featured</span><?php endif; ?>
                <h1><?php echo e($product->product_name); ?></h1>

                <div class="product-rating">
                    <div class="stars">
                        <?php $avgRating = round($product->reviews->where('is_approved',true)->avg('rating') ?? 0); ?>
                        <?php for($i=1;$i<=5;$i++): ?><i class="fas fa-star" style="<?php echo e($i > $avgRating ? 'color:#ddd' : 'color:var(--gold)'); ?>"></i><?php endfor; ?>
                    </div>
                    <span><?php echo e($product->reviews->where('is_approved',true)->count()); ?> reviews</span>
                </div>
                ...
                <form action="<?php echo e(route('cart.add')); ?>" method="POST" id="add-to-cart-form">
                    ...
                    <div class="size-options">
                        ...
                        <button type="button"
                            class="size-btn <?php echo e($size->pivot->stock_qty == 0 ? '' : ''); ?>"
                            data-size-id="<?php echo e($size->size_id); ?>"
                            data-price="<?php echo e($size->pivot->price); ?>"
                            data-stock="<?php echo e($size->pivot->stock_qty); ?>"
                            <?php echo e($size->pivot->stock_qty == 0 ? 'disabled' : ''); ?>

                            onclick="selectSize(this)">
                            <?php echo e($size->size_name); ?>

                            <?php if($size->pivot->stock_qty == 0): ?><br><small style="font-size:.65rem;">Out</small><?php endif; ?>
                        </button>
        ...
        <!-- TABS -->
        <div class="tab-nav">
            <button class="tab-btn active" onclick="switchTab('reviews', this)">Reviews (<?php echo e($product->reviews->where('is_approved',true)->count()); ?>)</button>
        ...
        <div id="tab-reviews" class="tab-content active">
            <?php if(auth()->guard()->check()): ?>
                <?php $myPending = $product->reviews->where('is_approved', false)->where('user_id', auth()->id())->first(); ?>
                <?php if($myPending): ?>
            ...
            <div class="review-item">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.5rem;">
                    <div style="display:flex;align-items:center;gap:.75rem;">
                        <div style="width:36px;height:36px;background:var(--gold);border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;color:#fff;font-size:.85rem;">
                            <?php echo e(strtoupper(substr($review->user->username ?? 'U', 0, 1))); ?>

                        </div>
                        <div>
                            <div style="font-weight:600;font-size:.875rem;"><?php echo e($review->user->username ?? 'Customer'); ?></div>
                            <div style="font-size:.75rem;color:var(--gray);"><?php echo e($review->created_at->format('M d, Y')); ?></div>
                        </div>
                    </div>
                    <div class="stars">
                ...
                <h4 style="font-size:.9rem;margin-bottom:.35rem;"><?php echo e($review->review_title); ?></h4>
                <p style="font-size:.875rem;color:var(--gray);line-height:1.7;"><?php echo e($review->description); ?></p>
            </div>
        ...
        <div id="tab-write-review" class="tab-content">
            <?php if(auth()->guard()->check()): ?>
            <form action="<?php echo e(route('reviews.store')); ?>" method="POST" style="max-width:600px;">
                <?php echo csrf_field(); ?>
                <input type="hidden" name="product_id" value="<?php echo e($product->product_id); ?>">
                <div style="margin-bottom:1.25rem;">
                    ...
                    <div style="display:flex;gap:.5rem;" id="star-rating">
                        ...
                        <label style="cursor:pointer;">
                            <input type="radio" name="rating" value="<?php echo e($i); ?>" style="display:none;" required>
                            <i class="fas fa-star" style="font-size:1.5rem;color:#ddd;transition:color .15s;" data-val="<?php echo e($i); ?>"></i>
                        </label>
                        <?php endfor; ?>
                    </div>
                </div>
    ...
    <div style="border-top:1px solid var(--border);padding-top:3rem;">
        ...
        <div class="related-grid">
            ...
            <div class="product-card">
                <a href="<?php echo e(route('shop.show', $rp->product_id)); ?>" style="text-decoration:none;">
                    <div class="product-card-img">
                        <?php if($rp->product_image): ?><img src="<?php echo e(asset('storage/'.$rp->product_image)); ?>" alt="<?php echo e($rp->product_name); ?>">
                        <?php else: ?> <i class="fas fa-tshirt"></i> <?php endif; ?>
                    </div>
                </a>
                <div class="product-card-body">
                    <h3 class="product-card-name"><?php echo e($rp->product_name); ?></h3>
                    <div class="product-card-price">
                        <?php if($rp->sizes->isNotEmpty()): ?> From $<?php echo e(number_format($rp->sizes->min('pivot.price'),2)); ?> <?php else: ?> N/A <?php endif; ?>
                    </div>
                </div>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<script>
const sizeData = <?php echo json_encode($product->sizes->mapWithKeys(fn($s) => [$s->size_id => ['price' => $s->pivot->price, 'stock' => $s->pivot->stock_qty]]), 512) ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\vesot\Downloads\Telegram Desktop\ecommerce-laravel-v6-fixed (2)\ecommerce-laravel-v6-fixed\ecommerce-laravel-fixed\resources\views/shop/show.blade.php ENDPATH**/ ?>